...
const webhookValidator = (req, res, next) => {
  ...
  if (!signature || !timestamp) {
    console.warn('Missing required headers: x-webhook-signature or x-webhook-timestamp.');
  ...
  if (Math.abs(currentTime - parseInt(timestamp, 10)) > 300) {
    console.warn(`Stale webhook: timestamp ${timestamp} is older than 5 minutes.`);
    return res.status(401).send('Unauthorized');
  }
  ...
  if (hmac !== signature) {
    console.warn(`Invalid webhook signature. Event type: ${req.body.event}, Timestamp: ${timestamp}`);
    return res.status(401).send('Unauthorized');
  }

  // Log event type, timestamp, and validation result
  console.log(`Webhook validated successfully. Event type: ${req.body.event}, Timestamp: ${timestamp}`);
